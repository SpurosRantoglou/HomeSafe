...
def alarm():
    ...
    #winsound.PlaySound("alarmsound.mp3", winsound.SND_FILENAME)
...
while True:
    # grab the current frame and initialize if room is empty or not text
    frame = vs.read()
    ...
    for c in cnts:
        ...
        # compute the bounding box for the countour, draw it on the frame, and update the text
        (x, y, w, h) = cv2.boundingRect (c)
        cv2.rectangle(frame,(x, y), (x + y, y + h), (0, 255, 0), 2)
        text = "Someone entered your house"
        #alarm()


    # draw the text abd timestamp on the frame
